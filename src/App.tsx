...
function App() {
  ...
  return (
    <div className="w-screen h-screen overflow-hidden relative select-none font-sans">
      ...
      {gameStatus === "instructions" && <InstructionsModal />}

      {/* 4. GAME OVER */}
      {gameStatus === "gameover" && (
        <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-red-900/80 backdrop-blur-md">
          <h2 className="text-6xl font-black text-white mb-4 drop-shadow-[0_4px_4px_rgba(0,0,0,0.5)]">
            ¡PARTIDA TERMINADA!
          </h2>
          <div className="bg-white p-8 rounded-2xl shadow-2xl text-center mb-8">
            <p className="text-slate-600 text-base uppercase tracking-widest font-black mb-2">
              PUNTUACIÓN FINAL
            </p>
          ...
          <div className="flex gap-4">
            <button
              ...
              className="bg-gradient-to-r from-red-400 to-red-600 hover:from-red-500 hover:to-red-700 text-white font-black py-4 px-8 rounded-full text-xl border-4 border-red-800 active:scale-95 transition-all flex items-center gap-2"
            >
              <svg
            ...
            <button
              onClick={startGame}
              className="bg-gradient-to-r from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 text-white font-black py-4 px-10 rounded-full text-xl border-4 border-green-800 active:scale-95 transition-all"
            >
              REINTENTAR
            </button>
          </div>
        ...
        <div className="absolute inset-0 z-[100] flex items-center justify-center bg-slate-900/95 p-8 overflow-auto">
          <div className="bg-white rounded-3xl p-8 max-w-4xl w-full text-slate-800">
            <h2 className="text-4xl font-black mb-6">
              Reporte Psicológico: MindShift
            </h2>
            <div className="space-y-4 max-h-[60vh] overflow-y-auto">
              {useGameStore.getState().mindshiftStats.map((stat, idx) => (
                <div
                  key={idx}
                  className={`p-4 rounded-xl border-l-8 ${stat.isCorrect ? "border-green-500 bg-green-50" : "border-red-500 bg-red-50"}`}
                >
                  ...
                  <div className="text-sm mt-2 grid grid-cols-2 gap-4">
                    <div>
                      <strong>Tiempo de Reacción:</strong> {stat.reactionTime}ms
                    </div>
                    <div>
                      <strong>Estímulo:</strong> {stat.emotion}
                    </div>
                    {stat.phase === 2 && (
                      <div>
                        <strong>¿Cayó en trampa visual?:</strong>{" "}
                        {stat.choseDistractor ? "Sí" : "No"}
                      </div>
                    )}
                  </div>
                </div>
