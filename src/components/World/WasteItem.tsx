...
export const WasteItem: React.FC<WasteItemProps> = ({ data }) => {
  ...
  useFrame((state) => {
    ...
    if (data.mindshiftDistractor && distractorMatRef.current) {
      ...
      distractorMatRef.current.emissiveIntensity =
        1 + Math.sin(time * 15) * 1.5;
      if (groupRef.current) {
      ...
      {data.type === "obstacle" ? (
        ...
        <mesh ref={meshRef} frustumCulled={false}>
          ...
          <mesh position={[0, -0.2, 1.01]}>
            ...
            {/* Etiqueta Mindshift */}
            {data.mindshiftLabelText && !data.englishOptionText && (
              <Text
                position={[0, 0, 0.01]}
                fontSize={0.19}
                color="black"
            ...
            {/* Etiqueta English — muestra la opción asignada a este carril */}
            {data.englishOptionText && (
              <Text
                position={[0, 0, 0.01]}
                fontSize={0.22}
                color="black"
