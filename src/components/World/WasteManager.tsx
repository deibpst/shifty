    ...
    useFrame((state, delta) => {
        ...
        if (useGameStore.getState().gameMode === 'mindshift') {
            if (currentWasteItem && lastSpawnedMindshiftItemId.current !== currentWasteItem.id) {
                ...
                config.colors.forEach((laneColor, laneIdx) => {
                    let text = '';
                    let isDistractor = false;
                    
                    if (phase === 1) {
                        const emos = ['FELIZ', 'TRISTE', 'ENOJADO', 'CON MIEDO'];
                        text = emos[laneIdx] || 'SORPRESA';
                    } else if (phase === 2) {
                        ...
                        text = (laneIdx < 2) ? 'POSITIVO' : 'NEGATIVO';
                    }
        ...
        objectsDataRef.current.forEach(obj => {
            ...
            if (obj.z > PLAYER_Z - 1 && obj.z < PLAYER_Z + 1) {
                ...
                if (dist < COLLISION_THRESHOLD && obj.laneIndex === currentLane) {
                    ...
                    if (useGameStore.getState().gameMode === 'mindshift') {
                         let isMatch = false;
                         let choseDistractor = !!obj.mindshiftDistractor;
                         
                         if (currentWasteItem) {
                             const phase = useGameStore.getState().mindshiftPhase;
                             if (phase === 1) {
                                 isMatch = (obj.mindshiftLabelText === currentWasteItem.mindshiftEmotion);
                             } else if (phase === 2) {
                                 isMatch = (obj.color === currentWasteItem.correctLaneColor);
                             } else {
                                 isMatch = (obj.mindshiftLabelText === currentWasteItem.mindshiftEmotion);
                             }
                         }
