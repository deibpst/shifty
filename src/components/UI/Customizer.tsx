...
const THEMES = [
  {
    ...
    text: "text-green-600",
    btn: "bg-green-500 hover:bg-green-600 shadow-green-700",
  ...
  },
  {
    ...
    text: "text-blue-600",
    btn: "bg-blue-500 hover:bg-blue-600 shadow-blue-700",
  ...
  },
  {
    ...
    btn: "bg-yellow-500 hover:bg-yellow-600 shadow-yellow-700",
  ...
  },
  {
    ...
    text: "text-red-600",
    btn: "bg-red-500 hover:bg-red-600 shadow-red-700",
...
export const Customizer: React.FC = () => {
  ...
  const visibleCharacters = AVAILABLE_CHARACTERS.slice(
    startIndex,
      ...
      >
              ...
              {visibleCharacters.map((char, index) => {
                ...
                return (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: -50 }}
                    className={`relative rounded-2xl overflow-hidden border-4 transition-all duration-300 group bg-white
                                            ${
                                              isSelected
                                                ? `${theme.border} ${theme.shadow} shadow-2xl scale-[1.05] ring-4 ring-offset-2 ${theme.name === "green" ? "ring-green-400" : "ring-blue-400"}`
                                                : "border-slate-200 shadow-sm hover:border-slate-400 hover:scale-[1.01]"
                                            }
                                            ${isLocked ? "grayscale opacity-70" : ""}
                                        `}
                  >
                    {/* 3D Preview */}
                    <div className="h-40 w-full bg-gradient-to-b from-slate-300 to-slate-200 relative">
                    ...
                    {/* Info & Action */}
                    <div
                      className={`p-4 text-center transition-colors duration-500 ${isSelected ? theme.bg : "bg-white"}`}
                    >
                      <h3 className="text-lg font-black text-slate-800 mb-1">
                        {char.name}
                      </h3>
                      <p className="text-sm text-slate-700 mb-3 min-h-[2rem] flex items-center justify-center font-semibold leading-tight">
                        {char.description}
                      </p>
                          ...
                          className={`w-full py-2.5 rounded-lg font-bold transition-all transform active:scale-95 text-sm
                                                        ${
                                                          isSelected
                                                            ...
                                                            : `${theme.btn} text-white border-2 ${theme.border} hover:scale-105 shadow-md`
                                                        }
                                                    `}
                        >
        ...
        <div className="p-3 flex items-center justify-between gap-3 z-20 shrink-0 bg-white border-t border-slate-100">
          ...
          <button
            ...
            className="flex-1 py-3 bg-red-500 hover:bg-red-600 text-white rounded-xl font-black text-base border-2 border-red-700 active:scale-95 transition-all"
          >
            SALIR
          </button>
          ...
          <button
            onClick={handleNext}
            className="flex-1 py-3 bg-yellow-400 hover:bg-yellow-500 text-yellow-900 rounded-xl font-black text-base border-2 border-yellow-700 active:scale-95 transition-all flex items-center justify-center gap-2"
          >
            <span>SIGUIENTE</span>
            <svg
